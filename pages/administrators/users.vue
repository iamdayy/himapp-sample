...
// Data
const { data: todos, pending } = await useLazyAsyncData<IUser[]>('todos', () => ($fetch as any)(`https://jsonplaceholder.typicode.com/todos${searchStatus.value}`, {
    query: {
        q: search.value,
        '_page': page.value,
        ...
        <UCard class="px-8 mt-6">
            <template #header>
                <UButton label="New" size="xl" class="text-2xl my-3 font-bold text-gray-700 dark:text-gray-700"
                    :ui="{ rounded: 'rounded-full' }" v-if="isAdmin || isDept" block />
            </template>
